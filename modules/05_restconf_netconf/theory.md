...
2.  Add the following functions below `get_restconf_data`:
    ...
    def get_memory_utilization():
        ...
        if data:
            ...
            memory_used = mem_stats.get('used-memory')
            memory_total = mem_stats.get('total-memory')
            return memory_used, memory_total
        return "N/A", "N/A"

    def get_interface_status():
        """Queries and returns interface operational status from IOS XE via RESTCONF."""
        path = "ietf-interfaces:interfaces" # Get all interfaces
        data = get_restconf_data(path)
        interfaces = []
        if data:
...
2.  Add the following code at the very end of the file (outside any Flask app routes):
    ...
    if __name__ == '__main__':
        ...
        print("Note: This will attempt to connect to the IOS XE router defined in API_INFO.")
...
4.  **Run the script** from your `network_automation_labs` directory:
    ...
    *Expected Output (if dummy IOS XE info is not replaced):*
    ...
    Note: This will attempt to connect to the IOS XE router defined in API_INFO.
    ...
    Fetching: https://YOUR_IOSXE_IP:443/restconf/data/ietf-interfaces:interfaces
    Error fetching ietf-interfaces:interfaces: HTTPSConnectionPool(host='YOUR_IOSXE_IP', port=443): Max retries exceeded with url: /restconf/data/ietf-interfaces:interfaces (Caused by NewConnectionError('<urllib3.connection.HTTPSConnection object at 0x...>: Failed to establish a new connection: [Errno 111] Connection refused'))
